...
void Part::addvect(int num, int *nnn) {
  if (vect) {
    ...
    // copy old dbc
    int i;
    for (i = 0; i < length; ++i) nd[i] = vect[i];

    // copy new dbc
    for (i = 0; i < num; ++i) nd[i + length] = nnn[i];
...
Partset::Partset(int initsize) {
  ...
  pmax = initsize;
  int i;
  for (i = 0; i < pmax; ++i) parts[i] = 0;
}

void Partset::partadd(int pn, int type, Part *el) {
  el->partnum = pn;
  el->parttype = type;
...
Part *Partset::getpart(int elnum) {
  int i;
  for (i = 0; i < pmax; ++i) {
    if (parts[i]->partnum == elnum) return parts[i];
  }
  return parts[elnum];
}

void Partset::partadd(int pn, int type, int n, int *el) {
  Part *p = new Part();
  p->addvect(n, el);
  partadd(pn, type, p);
}
