...
Element *
EightNodeBricko::clone()
{
	return new EightNodeBricko(*this);
}


int
EightNodeBricko::numNodes()
{
 	return 8;
}

int*
EightNodeBricko::nodes(int *p)
{
 	if(p == 0) p = new int[8];
 	p[0] = nn[0];
 	p[1] = nn[1];
...
double
EightNodeBricko::getfm()
{
    return elsta->fm;
}

double
EightNodeBricko::getmass()
{
    return elsta->mass;
}

int
EightNodeBricko::numDofs()
{
 	return 24;
}

int
EightNodeBricko::getDimNumber()
{
 return 3;
}

void
EightNodeBricko::renum(CoordSet& cs)
{
...
int
EightNodeBricko::rerenum(CoordSet& cs)
{
    int i;
    for(i =0; i < 8; ++i){
...
int
EightNodeBricko::getVol(CoordSet& cs, DynOps& ops)
{
    int i,j;
    Node *nd;
    double xyz[8][3], evol;
...
void
EightNodeBricko::setvol0()
{
    elsta->vol0 = elsta->vol;
}

double
EightNodeBricko::getvol0()
{
    return elsta->vol0;
}

void
EightNodeBricko::setMass(CoordSet &cs,double evol)
{
    int i;
...
void
EightNodeBricko::setinistr(double *istress)
{
    int i;
    for(i = 0; i < 6; ++i)
    {
...
void
EightNodeBricko::calcde(DynOps &ops,Solver &sol,CoordSet& cs)
{
    int i,j;
    double v[8][3];
    Node* nd;
    ...
    for (i = 0; i< 3;++i){
        for(j=0;j<3;++j){
            dij[i][j] = v17[i]*pkxj[0][j]+v28[i]*pkxj[1][j]+v35[i]*pkxj[2][j]+v46[i]*pkxj[3][j];
        }
        ops.de[i] = dij[i][i];
    }
...
void
EightNodeBricko::constructMat(DynOps& ops, Solver& sol)
{
    int i;
    prop->calcstress((double*)elsta->stress, (double*) ops.de, (double*) hisv ,sol.dt, sol.currentTime , plas, elfail);
}

void
EightNodeBricko::large(DynOps& ops)
{
    double q[3], rot[6];
    int i;
	...
	rot[1] = -q[2] + q[1];
	rot[2] = -q[1] - q[2];
	rot[4] = ops.vort[1]*(elsta->stress[2]-elsta->stress[1]) - ops.vort[2]*elsta->stress[3] - ops.vort[0]*elsta->stress[5];
	rot[5] = ops.vort[2]*(elsta->stress[2] - elsta->stress[0]) - ops.vort[1]*elsta->stress[3] + ops.vort[0]*elsta->stress[4];
	rot[3] = ops.vort[0]*(elsta->stress[1] - elsta->stress[0]) + ops.vort[1]*elsta->stress[4] + ops.vort[2]*elsta->stress[5];
...
void
EightNodeBricko::updatenodalforce(CoordSet& cs, DynOps& ops)
{
    int i,j,k;
    double pkxj[4][3],pkxjv[4][3];
    /*
    double nf[8][3];
    _FORTRAN(nodalforce)((double*)nf, (double*) ops.pkxj,(double*) elsta->stress, elsta->vol);*/
    double nf[3][8];
    ...
    for(i = 0; i < 4; ++i){
        nf[0][i] = (elsta->stress[0]*pkxjv[i][0] + elsta->stress[3]*pkxjv[i][1] + elsta->stress[5]*pkxjv[i][2]);
        nf[1][i] = (elsta->stress[1]*pkxjv[i][1] + elsta->stress[3]*pkxjv[i][0] + elsta->stress[4]*pkxjv[i][2]);
        nf[2][i] = (elsta->stress[2]*pkxjv[i][2] + elsta->stress[4]*pkxjv[i][1] + elsta->stress[5]*pkxjv[i][0]);
    }
...
void
EightNodeBricko::updatea(CoordSet &cs,Curveset &mm, Solver &sol, damping &dp, StaticSolver &sts)
{
    int i;
    for(i = 0; i < 8 ; ++i){
...
int
EightNodeBricko::getouttype()
{
    return 12;
}

double*
EightNodeBricko::getoutstress(double* p)
{
...
double*
EightNodeBricko::getoutstrain(double* p)
{
 	if(p == 0) p = new double[6];
 	p[0] = elsta->strain[0];
 	p[1] = elsta->strain[1];
...
int
EightNodeBricko::sethistory(int n)
{
    int i;
    delete[] hisv;
    hisv = new double[n];
...
double*
EightNodeBricko::getouthis(double* p)
{
    int i;
...
void
EightNodeBricko::hourglassctrl(DynOps &ops,CoordSet& cs, int& flg, double& para)
{
    int i,j,k;
    ...
    //    _FORTRAN(hourglassforce)((double*) v, (double *) xyz, ops.pkxj, evol, density, ws,flg, para, (double*) force);
    switch(flg){
    case 0:
        break;
    case 1:
...
double
EightNodeBricko::EleTimeStep(CoordSet& cs)
{
    int i,j;
    Node *nd;
    double xyz[8][3],evol;
...
int
EightNodeBricko::ResetData(double *d, int nhis){
    int i;
    sethistory(nhis);
    for(i = 0 ; i < 6; ++i){
        elsta->strain[i] = d[i];
...
double*
EightNodeBricko::getlocal(CoordSet& cs){
  Node* nd1 = cs[nn[0]];
  Node* nd2 = cs[nn[1]];
  Node* nd3 = cs[nn[2]];
  Node* nd4 = cs[nn[3]];
  ...
  double xyz[8][3];
  	...
  	xyz[4][0] = nd5->x;xyz[4][1] = nd5->y; xyz[4][2] = nd5->z;
  	xyz[5][0] = nd6->x;xyz[5][1] = nd6->y; xyz[5][2] = nd6->z;
  	xyz[6][0] = nd7->x;xyz[6][1] = nd7->y; xyz[6][2] = nd7->z;
  	xyz[7][0] = nd8->x;xyz[7][1] = nd8->y; xyz[7][2] = nd8->z;
    int i,j;
