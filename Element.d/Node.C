...
int
Node::setdisp(int dir, int cnum, double disp)
{
    if(dir == 1)
    {
        cun[0] = cnum;
...
void
Node::updatea(Curveset &mm, Solver &sol, damping &dp, StaticSolver &sts)
{
    int i;
    int deb;
    double t[3];
...
Node::updateu(Solver &sol){
        ...
        for(i = 0 ; i < 3 ; ++i)
    {
        ...
        u[i] += du[i];
        if(fabs(du[i])>1.0){
            fprintf(stderr,"WARNING, %d displacement to large!direction = %d,v = %e, a = %e\n",nodenum,i,v[i],a[i]);
            }
     }
        x +=du[0];
        y +=du[1];
...
Node::updateubf()
{
    ...
    ubfr = fabs(sqrt(ubf[0]*ubf[0] + ubf[1]*ubf[1] +ubf[2]*ubf[2]) - sqrt(temp[0]*temp[0] + temp[1]*temp[1] +temp[2]*temp[2]) );
                }
