...
void Matset::propadd(StructProp *p) {
  int newsize = pmax + 1;
  proP *pr = new proP[newsize];
  int i;
  for (i = 0; i < pmax; ++i) pr[i] = props[i];
  pmax = newsize;
  pr[pmax - 1] = 0;
  delete[] props;
  props = pr;
...
StructProp *Matset::getmat(int mnum) {
  int i;
  for (i = 0; i < pmax; ++i) {
    if (props[i]->matnum == mnum) return props[i];
  }
}
