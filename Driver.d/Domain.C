...
Domain::Domain(): eset(*(new Elemset)),nodes(*(new CoordSet)),pset(*(new Partset)),
                  mset(*(new Matset)), inistr(emptyst), pres(emptypr), attrib(emptyat),pattrib(emptyat),
                  cset(*(new Curveset))
...
int
Domain::setDirichlet(int _numDirichlet, BCond *_dbc)
{
...
int
Domain::setNeuman(int _numNeuman, BCond *_nbc)
{
if(nbc) {
    // Allocate memory for correct number of dbc
...
int
Domain::setfix(int _numfixed, BCFixed *_nfixed)
{
if(nfixed) {
    // Allocate memory for correct number of dbc
   ...
   // copy new dbc 
   for(i=0; i<_numfixed; ++i)
     nd[i+numfixed] = _nfixed[i];
...
int
Domain::setoutfile(char* of){
 finfo.outputname = of;
}

int
Domain::setIAcc(int _numIAcc, BCond *_iAcc)
{
 if(iAcc) {
    // Allocate memory for correct number of dbc
   ...
   // copy new dbc 
   for(i=0; i<_numIAcc; ++i)
     nd[i+numIAcc] = _iAcc[i];
...
int
Domain::setinistress(int num, double *stress){
...
void
Domain::setinfilepath(char* inp){
    strcpy(finfo.infpath , inp);
}

void
Domain::setrefilepath(char* rep){
    strcpy(finfo.refpath , rep);
}

void
Domain::addnout(int noutnum){
 if(nout) {
    // Allocate memory for correct number of dbc
...
void Domain::setlarge()
{
    jaumannflg = 1;
}

void
Domain::setgrav(double x, double y, double z)
{
...
void
Domain::setatt(int elnum, int mnum)
{
...
void
Domain::setpatt(int pnum, int mnum)
{
...
int
Domain::inisolver(SolverInfo& sinfo)
{
...
int
Domain::SetCurrentTime(double t){
    solv.currentTime = t;
    return 0;
}

int
Domain::ResetSolver(int flg, double *st){
...
Domain::preProcess()
{
 ...
 for(i = 0; i< numDirichlet; ++i){
     ...
     nodes.getNode(dbc[i].nnum).setdisp(dir, cnum, value);        //set disp and defined
...
// output initial state
  ...
  I = TECINI112((char*)"DATABASE OF DCP",
                (char*)tecvarname,
                (char*)tecoutname,
                (char*)".",
                &FileType, 
...
// creat file
    ...
    // ==============================================
          ...
          if(ointer.outfiletype == 2){
             dynatecpoutput(outfilenum, solv.currentTime, varlocation,varpositive, varshare);
  ...
  // calc strain stress inter force  
    while((solv.tmax > 0.0)?(solv.currentTime <= solv.tmax) : (sts.termsign == 0))  {
        // fenqupolie();
 ...
 for(i =0; i < numpress; ++i){
  ...
  //calc strain and stress and force
           for(i = 0; i < exactNumEle; ++i)
           {
               ...
               if(jaumannflg == 1){el->large(dynops); }
               el->constructMat(dynops,solv);
               
               el->updatenodalforce(nodes,dynops);
               if(hourglassflg != 0){ el->hourglassctrl(dynops,nodes, hourglassflg,hourglasspara); }
             }
...
void
Domain::dynaplotoutput(int outfilenum, double ct)
{
    FILE* rout;
             ...
             for(i = 0; i < numnout; ++i){
                 if(nout[i] == 1){
                     ...
                     for(j = 0; j < numnodes; ++j){
                         fprintf(rout,"%7.3e\t%7.3e\t%7.3e\n",nodes[j]->u[0],nodes[j]->u[1],nodes[j]->u[2]);
                             }
                     fprintf(rout,"\n"); 
                 }
                 ...
                 if(nout[i] == 2){
                     ...
                     for(j = 0; j < numnodes; ++j){
                         fprintf(rout,"%7.3e\t%7.3e\t%7.3e\n",nodes[j]->interf[0],nodes[j]->interf[1], nodes[j]->interf[2]);
                             }
                    fprintf(rout,"\n");    
                      }
                  if(nout[i] == 3){
                     ...
                     for(j = 0; j < numnodes; ++j){
                         fprintf(rout,"%7.3e\t%7.3e\t%7.3e\n",nodes[j]->v[0],nodes[j]->v[1], nodes[j]->v[2]);
                             }
                    fprintf(rout,"\n");    
                      }
                   if(nout[i] == 4){
                     ...
                     for(j = 0; j < numnodes; ++j){
                         fprintf(rout,"%7.3e\t%7.3e\t%7.3e\n",nodes[j]->a[0],nodes[j]->a[1], nodes[j]->a[2]);
                             }
                    fprintf(rout,"\n");    
                      }
                     if(nout[i] == 5){
                     ...
                     for(j = 0; j < numnodes; ++j){
                         fprintf(rout,"%7.3e\t%7.3e\t%7.3e\n",nodes[j]->extf[0],nodes[j]->extf[1], nodes[j]->extf[2]);
                             }
                    fprintf(rout,"\n");    
                      }
                 }
             ...
             for(i = 0; i < numeout; ++i){
                 ...
                 if(eout[i] == 3){
                     ...
                     for(j = 0; j < exactNumEle; ++j){
                         ...
                         mises = sqrt(1./2. *(stress[0]*stress[0]+stress[1]*stress[1]+stress[2]*stress[2] + 2.* stress[3] * stress[3] + 2. * stress[4]* stress[4] + 2. * stress[5] * stress[5] ) );
                         //jaco_(stress,prinstre,stredir);
            ...
            //            }
                 ...
                 for(i =0 ;i <1; ++i){
                     ...
                     ccenter = el->getlocal(nodes);
                         ...
                         fprintf(heout,"%e,", sqrt(1./2. *(stress[0]*stress[0]+stress[1]*stress[1]+stress[2]*stress[2] + 2.* stress[3] * stress[3] + 2. * stress[4]* stress[4] + 2. * stress[5] * stress[5] ) ));
...
int
Domain::dynastatoutput(int outfilenum, double ct){
    FILE* sout;
...
int
Domain::ResetNData(double* dt, int* nt ,int n){
    int i , j ;
        Node* nd = nodes[n];
...
int
Domain::ResetEData(double *dt,int hn, int en){
    int i, j;
    Element *el = eset[en];
      if(el == 0) {
      fprintf(stderr,"\n *** WARNING: Element was not existent element %d\n",i);
...
int
Domain::setbulkvisco(double q1, double q2){
...
int
Domain::setdamping(int type, double para){
    if(type == 1){
...
int
Domain::staticterm(double flg){
    sts.staticflg = 1;
    sts.miniE = flg;
}

int
Domain::TECPLOT_VNN(int &num, char* name){
    int i,j;
    num =3;
    sprintf(name,"X Y Z");
    num +=1;
    ...
    for(i = 0; i < numeout; ++i){
        ...
        case 1:
            num += 6 ;
            sprintf(name,"%s XSTRESS YSTRESS ZSTRESS XYSTRESS YZSTRESS XZSTRESS", name);
            break;
        case 2:
            num += 6;
            sprintf(name,"%s XSTRAIN YSTRAIN ZSTRAIN XYSTRAIN YZSTRAIN XZSTRAIN", name);
            break;
        case 3:
            num += 16;
            sprintf(name,"%s HISTORY1 HISTORY2 HISTORY3 HISTORY4 HISTORY5 HISTORY6 HISTORY7 HISTORY8 HISTORY9 HISTORY10 HISTORY11 HISTORY12 HISTORY13 HISTORY14 HISTORY15 HISTORY16", name);
            break;
        case 4:
            num += 3;
...
int
Domain::dynatecpoutput(int onum, double ct, int *vl, int *pf, int *shv){

  INTEGER4 Debug,I,J,K,L,II,III,NPts,NElm,DIsDouble,VIsDouble,IMax,JMax,KMax;
  ...
  INTEGER4 StrandID, ParentZone, TotalNumFaceNodes,NumConnectedBoundaryFaces, TotalNumBoundaryConnections ;
  //packelemdelete();
  ZoneType  = 5;
  IMax      = numnodes;
  JMax      = exactNumEle;
    ...
    for(i = 0; i < numeout; ++i){
        ...
        case 1:
            ...
            for(k = 0 ; k < exactNumEle ; ++k){
                ...
                for(j = 0; j < 6 ; ++j)
                eletemp[j*exactNumEle+k] = stress[j];
            }
            II = exactNumEle * 6;
            I = TECDAT112(&II, &eletemp[0], &DIsDouble);
            break;
        case 2:
            ...
            for(k = 0 ; k < exactNumEle ; ++k){
                ...
                for(j = 0; j < 6 ; ++j)
                eletemp[j*exactNumEle+k] = strain[j];
            }
            II = exactNumEle * 6;
            I = TECDAT112(&II, &eletemp[0], &DIsDouble);
            break;
        case 3:
            ...
            for(k = 0 ; k < exactNumEle ; ++k){
                ...
                for(j = 0; j < 16 ; ++j)
                eletemp[j*exactNumEle+k] = hisv[j];
            }
            II = exactNumEle * 16;
            I = TECDAT112(&II, &eletemp[0], &DIsDouble);
            break;
        case 4:
            ...
            for(k = 0 ; k < exactNumEle ; ++k){
                ...
                princ_stress(stress, stredir, prinstre);
                for(j = 0; j < 3 ; ++j)
                eletemp[j*exactNumEle+k] = prinstre[j];
            }
            II = exactNumEle * 3;
            I = TECDAT112(&II, &eletemp[0], &DIsDouble);
            break;
        case 5:
            ...
            for(k = 0 ; k < exactNumEle ; ++k){
                stress = packedEset[k]->getoutstress(stress);
                         ...
                         stress[1]=stress[1]-dpval;
                         stress[2]=stress[2]-dpval;
                eletemp[k] = sqrt(1./2. *(stress[0]*stress[0]+stress[1]*stress[1]+stress[2]*stress[2] + 2.* stress[3] * stress[3] + 2. * stress[4]* stress[4] + 2. * stress[5] * stress[5] ) );}
            II = exactNumEle;
            I = TECDAT112(&II, &eletemp[0], &DIsDouble);
            break;
        case 6:
            ...
            for(k = 0 ; k < exactNumEle ; ++k){
                ...
                for(j = 0; j < 3 ; ++j)
                eletemp[j*exactNumEle+k] = ccenter[j];
            }
            II = exactNumEle*3;
            I = TECDAT112(&II, &eletemp[0], &DIsDouble);
            break;            
...
Domain::exportflac(){
    ...
    for(i = 0; i < numnodes; ++i){
        fprintf(flacout,"G %d  %f  %f  %f\n",nodes[i]->nodenum, nodes[i]->x,nodes[i]->y,nodes[i]->z); 
    }
   ...
   Element *ele = eset[i];
        ...
        fprintf(flacout,"%d %d %d %d %d %d\n",nnn[3],nnn[4],nnn[2],nnn[7],nnn[5],nnn[6]);
    }
        delete[] nnn;
     int count =0;
     int l;
...
Domain::fenqupolie()
{
    ...
    if(fq->step ==0){
        ...
        if((fq->rkey[0]==0)&&(solv.currentTime >= (fq->ftime[0]+fq->oktime[0]))){
           for(i = 0 ; i< numele; ++i){
            Element* el = eset[i];
            el->elfail = 1;                
        }
        ...
        if((fq->rkey[1]==0)&&(solv.currentTime >= (fq->ftime[1]+fq->oktime[1]))){
           for(i = 0 ; i< numele; ++i){
            Element* el = eset[i];
            el->elfail = 1;                
        }
...
int
Domain::setfenqu(double d1, double d2,double d3,double d4){
    int i;
    fq->step = 0;
    fq->rstep =0;
    for(i = 0; i < 2; ++i){
...
int
Domain::settimestep(int timesteptype,  double val){
       ...
       for(i = 0; i <exactNumEle;++i)
        {
            ...
            if(gravity[0]||gravity[1]||gravity[2]){
                fm += el->getmass()*(fabs(gravity[0]+fabs(gravity[1])+fabs(gravity[2])));
            }
        }
