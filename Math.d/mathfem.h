...
/// Returns the signum of given value (if value is < 0 returns -1, otherwise
/// returns 1)
inline double sgn(double i) { return (i < 0. ? -1. : 1.); }

/// Returns the signum of given value (i = 0 returns 0, i < 0 returns -1, i > 0
/// returns 1)
 ...
 * The coefficients a to d gives the equation @f$ a x^3 + b x^2 + c x + d =
 * 0@f$.
 * @param a Coefficient
 * @param b Coefficient
 * @param c Coefficient
...
/**
 * Solves cubic equation for real roots, assuming that if cubic polynomial given
 * then the only possibility is that only three real roots exists. But also
 * accepts cubic coefficient degenerated to quadratic or linear equation. This
 * is used by algorithms for computing principal strain/stresses to overcome
 * rounding errors. The coefficients a to d gives the equation @f$ a x^3 + b x^2
 * + c x + d = 0@f$.
 * @param a Coefficient
 * @param b Coefficient
 * @param c Coefficient
 ...
 *
 * golden section search routine for finding the minimum of given function
 * represented by functor f. Input parameters: ax, bx, cx -> three x-coordinates
 * bracketing the minima (ax < bx < cx and f(bx) < f(ax) and f(bx) < f(cx)) tol
 * - tolerance Output parameters: xmin coordinate of minima return value - the
 * minimum found
 *
 ...
 */
template <class T>
double gss(double ax, double bx, double cx, const T &f, double tol,
           double &xmin) {
  int ii = 0;
...
template <class T>
double brent(double ax, double bx, double cx, const T &f, double tol,
             double &xmin) {
  int ii;
  double x_left = ax, x_right = cx;
  double x, x_midpoint, v, w, u, tol1, tol2, p, q, r, e_tmp, d = 0.0, fx, fv,
                                                             fw, fu;
  double e = 0.0;
  ...
  for (ii = 1; ii <= MATHFEM_BRENT_MAXITER; ii++) {
    ...
    if (fabs(e) > tol1) {
      ...
      if (fabs(p) < fabs(0.5 * q * e_tmp) && p < q * (x - x_left) &&
          p < q * (x_right - x)) {
        d = p / q;
        u = x + d;
